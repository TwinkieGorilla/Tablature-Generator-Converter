...
const initializeAnalyzer = audioSource => {
  analyzer = Meyda.createMeydaAnalyzer({
    ...
    callback: ({
      ...
      newSpectrum = powerSpectrum // populate spectrum

      if (spectralCentroid) { brightness = 1024 / (1024 * spectralCentroid) }
      if (spectralSpread) { currentEnergy = 0.87 + (spectralSpread / 1024) } 
    }
  }) 
  analyzer.start()
}
